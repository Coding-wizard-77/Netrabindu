...
export const ANPREventCard: React.FC<ANPREventCardProps> = ({ event, onInspect }) => {
  ...
  return (
    <div className="bg-[#0f172a] border border-slate-800 hover:border-cyan-500/50 rounded-xl p-3.5 transition-all duration-200 shadow-sm hover:shadow-cyan-950/30 flex flex-col justify-between gap-3 group">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5 text-xs text-slate-400 font-mono">
          <Camera className="w-3.5 h-3.5 text-cyan-400" />
          <span className="font-bold text-slate-200">{event.camera_code || 'CAM-01'}</span>
        </div>
      ...
      <div className="flex items-center justify-between bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80">
        ...
        <div className="text-right font-mono">
          ...
          <div className="text-xs font-bold text-emerald-400">{confidencePercent}%</div>
        </div>
      </div>

      {/* Visual Crop Preview */}
      <div className="grid grid-cols-2 gap-2 h-20">
        <div className="relative rounded overflow-hidden bg-black border border-slate-800 flex items-center justify-center">
          {event.evidence.plate_crop_uri ? (
            <img src={event.evidence.plate_crop_uri} alt="Plate Crop" className="w-full h-full object-contain" />
          ) : (
            <span className="text-[9px] font-mono text-slate-600">Plate Crop</span>
          )}
        </div>
        <div className="relative rounded overflow-hidden bg-black border border-slate-800 flex items-center justify-center">
          {event.evidence.vehicle_crop_uri ? (
            ...
            <span className="text-[9px] font-mono text-slate-600">Vehicle Crop</span>
          )}
        </div>
      </div>
      ...
      <div className="pt-2 border-t border-slate-800/60 flex items-center justify-between text-[11px] text-slate-400 font-mono">
        ...
        {onInspect && (
          <button
            ...
            className="text-cyan-400 hover:text-cyan-300 font-medium flex items-center gap-1 group-hover:underline"
          >
