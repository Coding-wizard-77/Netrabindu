...
export const PCRVanDispatcher: React.FC<PCRVanDispatcherProps> = ({
  ...
  return (
    <div className="rounded-xl border border-navy-700 bg-navy-900/90 p-4 space-y-3">
      <div className="flex items-center justify-between border-b border-navy-800 pb-2">
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">Dispatch nearest patrol intercept unit</p>
          </div>
        </div>
      ...
      {/* Target Info */}
      <div className="rounded bg-navy-950 p-2.5 text-xs font-mono border border-navy-800/80">
        <div className="flex items-center justify-between">
          <span className="text-slate-400">TARGET:</span>
          <span className="font-bold text-amber-400">{targetPlate}</span>
        </div>
        <div className="flex items-center justify-between mt-1 text-[11px]">
          <span className="text-slate-400">LOCATION:</span>
          <span className="text-slate-200">{targetLocation}</span>
        </div>
      </div>

      {/* Available Units Selection */}
      <div className="space-y-1.5">
        <label className="text-[10px] font-mono text-slate-400 font-bold uppercase">Select Intercept Patrol Unit</label>
        {vans.map((v) => (
          <div
            key={v.callsign}
            ...
            className={`flex items-center justify-between p-2 rounded-lg cursor-pointer border transition-colors ${
              selectedVan === v.callsign
                ...
                : 'border-navy-800 bg-navy-950/60 text-slate-300 hover:bg-navy-800'
            }`}
          >
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <div className="text-[10px] text-slate-400">{v.officer}</div>
              </div>
            </div>
            <div className="text-right font-mono text-[11px]">
              <div className="text-amber-400 font-bold">ETA: {v.eta}</div>
              <div className="text-[10px] text-slate-400">{v.dist}</div>
            </div>
          </div>
        ))}
      </div>
