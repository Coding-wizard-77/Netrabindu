...
export const IncidentHotspotMatrix: React.FC = () => {
  ...
  return (
    <div className="glass-panel rounded-2xl p-5 border border-navy-700/80 shadow-2xl h-full flex flex-col justify-between">
      <div>
        <div className="flex items-center justify-between border-b border-navy-800 pb-3 mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] font-mono text-slate-400">High-frequency Red Notice corridors</p>
            </div>
          </div>
          <span className="text-[10px] font-mono font-bold text-amber-400">TOP 5 NODES</span>
        </div>
        ...
        <div className="space-y-2.5">
          {hotspots.map((h) => (
            <div
              key={h.rank}
              className="p-2.5 rounded-xl bg-navy-950/80 border border-navy-800/80 hover:border-amber-500/40 transition-colors flex items-center justify-between text-xs font-mono"
            >
              <div className="flex items-center gap-2.5">
                <span className="h-6 w-6 rounded-lg bg-navy-850 flex items-center justify-center font-bold text-slate-300 border border-navy-700 text-[11px]">
                  #{h.rank}
                </span>
                <div>
                  <div className="font-bold text-slate-200 truncate max-w-[160px] sm:max-w-[200px]">
                    {h.name}
                  </div>
      ...
      <div className="mt-4 pt-3 border-t border-navy-800 flex items-center justify-between text-[11px] font-mono text-slate-400">
        <span className="flex items-center gap-1">
          <TrendingUp className="w-3.5 h-3.5 text-emerald-400" /> SG Highway Activity +18%
        </span>
        <span className="text-cyan-400 font-bold">Auto-Sync 30s</span>
      </div>
    </div>
  );
};
